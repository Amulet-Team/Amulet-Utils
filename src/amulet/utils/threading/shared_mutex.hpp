...
public:
    ...
    void lock_shared() ASTD_ACQUIRE_SHARED((*this)) ASTD_ACQUIRE_SHARED(shared_capability) ASTD_NO_THREAD_SAFETY_ANALYSIS { std::shared_mutex::lock_shared(); }
    ...
    [[nodiscard]] bool try_lock_shared() ASTD_TRY_ACQUIRE_SHARED(true, (*this)) ASTD_TRY_ACQUIRE_SHARED(true, shared_capability) ASTD_NO_THREAD_SAFETY_ANALYSIS { return std::shared_mutex::try_lock_shared(); }
    void unlock() ASTD_RELEASE_UNIQUE((*this)) ASTD_NO_THREAD_SAFETY_ANALYSIS { std::shared_mutex::unlock(); }
    void unlock_shared() ASTD_RELEASE_SHARED((*this)) ASTD_RELEASE_SHARED(shared_capability) ASTD_NO_THREAD_SAFETY_ANALYSIS { std::shared_mutex::unlock_shared(); }
};
...
public:
    ...
    explicit shared_lock(Mutex& m) ASTD_ACQUIRE_SHARED(m, m.shared_capability)
        : std::shared_lock<Mutex>(m) {}

    shared_lock(Mutex& m, std::defer_lock_t) noexcept ASTD_EXCLUDES(m, m.shared_capability)
        : std::shared_lock<Mutex>(m, std::defer_lock) {}

    shared_lock(Mutex& m, std::adopt_lock_t) ASTD_REQUIRES_UNIQUE_OR_SHARED(m, m.shared_capability)
    ...
    shared_lock(Mutex& m, std::try_to_lock_t) ASTD_MAYBE_ACQUIRE_SHARED(m, m.shared_capability)
        : std::shared_lock<Mutex>(m, std::try_to_lock) {}

    template< class Rep, class Period >
    shared_lock(Mutex& m, const std::chrono::duration<Rep, Period>& timeout_duration) ASTD_MAYBE_ACQUIRE_SHARED(m, m.shared_capability)
        : std::shared_lock<Mutex>(m, timeout_duration) {}

    template< class Clock, class Duration >
    shared_lock(Mutex& m, const std::chrono::time_point<Clock, Duration>& timeout_time) ASTD_MAYBE_ACQUIRE_SHARED(m, m.shared_capability)
    ...
    shared_lock( shared_lock&& other ) noexcept ASTD_MOVE_CAPABILITIES(other, (*this))
        : std::shared_lock<Mutex>(std::forward<shared_lock>(other)) {}

    shared_lock& operator=( shared_lock&& other ) noexcept ASTD_MOVE_CAPABILITIES(other, (*this))
    {
        std::shared_lock<Mutex>::operator=(std::forward<shared_lock>(other));
        return *this;
    }

    void swap( shared_lock& other ) noexcept ASTD_SWAP_CAPABILITIES(other, (*this)) {
...
void swap( astd::shared_lock<Mutex>& lhs,
           astd::shared_lock<Mutex>& rhs ) noexcept ASTD_SWAP_CAPABILITIES(lhs, rhs) {
    lhs.swap(rhs);
}
