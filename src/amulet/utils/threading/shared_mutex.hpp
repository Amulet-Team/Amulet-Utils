...
public:
    ...
    void lock_shared() ASTD_ACQUIRE_SHARED(this) ASTD_ACQUIRE_SHARED(shared) ASTD_NO_THREAD_SAFETY_ANALYSIS { std::shared_mutex::lock_shared(); }
    ...
    [[nodiscard]] bool try_lock_shared() ASTD_TRY_ACQUIRE_SHARED(true, this) ASTD_TRY_ACQUIRE_SHARED(true, shared) ASTD_NO_THREAD_SAFETY_ANALYSIS { return std::shared_mutex::try_lock_shared(); }
    void unlock() ASTD_RELEASE_UNIQUE(this) ASTD_NO_THREAD_SAFETY_ANALYSIS { std::shared_mutex::unlock(); }
    void unlock_shared() ASTD_RELEASE_SHARED(this) ASTD_RELEASE_SHARED(shared) ASTD_NO_THREAD_SAFETY_ANALYSIS { std::shared_mutex::unlock_shared(); }
};
