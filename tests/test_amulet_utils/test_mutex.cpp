...
void TestSharedMutex::test_lock_unlock_shared(){
    m.lock(); // expected-note {{mutex acquired here}}
    v += 1;
    m.unlock_shared(); // expected-error {{releasing mutex 'm' using shared access, expected exclusive access}}
}

void TestSharedMutex::test_lock_shared_unlock(){
    m.lock_shared(); // expected-note {{mutex acquired here}}
    auto a = v;
...
void TestSharedMutex::test_lock_shared_lock_shared(){
    m.lock_shared(); // expected-note 2 {{mutex acquired here}}
    m.lock_shared(); // expected-error {{acquiring mutex 'm' that is already held}}
} // expected-error {{mutex 'm' is still held at the end of function}}
...
void TestSharedMutex::test_try_lock_shared(){
    auto locked = m.try_lock_shared(); // expected-note {{mutex acquired here}}
    if (locked){
        auto a = v;
    } else {
...
void TestSharedMutex::test_try_lock_shared_unlock_shared_2(){
    ...
    m.unlock_shared(); // expected-error {{mutex 'm' is not held on every path through here}} expected-error {{releasing mutex 'm' that was not held}}
}
...
void TestSharedMutex::test_lock_shared_try_lock_shared(){
    m.lock_shared(); // expected-note 2 {{mutex acquired here}}
    auto locked = m.try_lock_shared(); // expected-error {{acquiring mutex 'm' that is already held}}
    if (locked){
        auto a = v;
        m.unlock_shared();
    }
...
void TestSharedLock::test_shared_adopt_constructor_unlocked(){
    astd::shared_lock lock(m, std::adopt_lock); // expected-error {{calling function 'shared_lock' requires holding mutex 'm'}}
}
...
void TestSharedLock::test_shared_adopt_constructor_locked_unique(){
    m.lock();
    astd::shared_lock lock(m, std::adopt_lock); // expected-error {{todo}}
}
